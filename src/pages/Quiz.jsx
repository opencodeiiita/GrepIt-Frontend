      ...
      <section className='w w-2/5 bg-gray-200/70 py-3 mt-1 px-2.5 rounded-xl flex flex-col h-1/4 shadow-sm shadow-gray-600'>
        ...
        <main className='grid grid-cols-2 justify-center items-center gap-y-2 gap-x-4 mx-2 mt-3'>
          <aside className='relative'>
            <span className={` absolute p-1 ${correctOption==="option1"?"bg-green-400 border-transparent":"bg-gray-200/70 border-black"} border top-1/4 left-2 hover:cursor-pointer`} onClick={()=>changeCorrectOption("option1")}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" dataSlot="icon"
               className={`${correctOption==="option1"?"visible":"invisible"} text-white w-3 h-3`}>
                <path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" />
              </svg>
            </span>
            <input className='text-center rounded-md p-3 bg-white/80 w-full' placeholder='1st Option'/>
          </aside>
          <input className='text-center rounded-md p-3 bg-white/80' placeholder='2nd Option'/>
          <input className='text-center rounded-md p-3 bg-white/80' placeholder='3rd Option'/>
          <input className='text-center rounded-md p-3 bg-white/80' placeholder='4th Option'/>
        </main>
      </section>
    </div>
  )
