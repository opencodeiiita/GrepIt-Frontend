...
const QuizCard=({quizProp,questionNumber,setQuestions})=>
{
  ...
  const deleteQuizCard=()=>
  {
    ...
    setQuestions((prev)=>[...prev].filter((question,index)=>index!==(questionNumber - 1)))
  }
  if(edit)
  return (
  ...
  return (
    <li className='px-4 py-3 border rounded-lg shadow-sm space-y-2  w-1/2'>
      <header className='flex justify-between border-b-2 pb-1'>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 hover:text-red-600 hover:cursor-pointer" onClick={deleteQuizCard}>
